            ...
            <div id="allowanceListContainer" className='w-full flex flex-col bg-component-white rounded-3xl overflow-hidden p-[40px] border border-solid border-dashcomponent-border'>
                ...
                <div className='flex justify-between h-[44px] mt-[25px]'>
                    <div onClick={handleFocusInput} className='cursor-text flex pl-[16px] pr-[16px] w-[250px] h-[40px] mt-auto items-center justify-between rounded-full bg-[#FDFDFE] outline-1 outline outline-[#E1E3E6] focus:outline-1 focus:outline-[#F86F4D]'>
                        <input spellCheck="false" ref={inputRef} disabled={!allowances || !isConnected} placeholder='Search' className='border-none outline-none bg-none h-[40px]' type="text" onInput={handleSearchInput} value={filters.searchValue} />
                        {filters.searchValue == "" ? 
                    ...
                    <div className='flex  gap-x-[10px]'>
                        <div onClick={allowances && isConnected ? handleDisplayUnlimitedOnly : undefined} className={'flex justify-center items-center gap-x-[10px] bg-[hsl(210,25%,100%)] px-[15px] rounded-[6px] shadow-[0_1px_2px_#A8B0BD10,0_3px_6px_#5D81B930]' + (allowances && isConnected ? ' cursor-pointer' : '')}>
                            <label id='unlimitedLabel' className={'flex items-center text-[14px]' + (allowances && isConnected ? ' cursor-pointer' : '')}>Unlimited only</label>
                            <div role="checkbox" aria-checked={filters.showUnlimitedOnly} className={'border-[1px] border-solid border-[#48494c] h-[14px] w-[14px] rounded-[3px] flex justify-center items-center' + (!filters.showUnlimitedOnly ? ' bg-[#eef0f2]' : ' bg-[#48494c]') + (allowances && isConnected ? ' cursor-pointer' : '')}>
                                {filters.showUnlimitedOnly && <img alt="checkbox image" src={checked}/>}
                            </div>
                        </div>
                        <div onClick={allowances && isConnected ? handleDisplayRevoked :undefined} className={'flex justify-center items-center gap-x-[10px] bg-[hsl(210,25%,100%)] px-[15px] rounded-[6px] shadow-[0_1px_2px_#A8B0BD10,0_3px_6px_#5D81B930]' + (allowances && isConnected ? ' cursor-pointer' : '')}>
                            <label id='revokedLabel' className={'flex items-center text-[14px]' + (allowances && isConnected ? ' cursor-pointer' : '')}>Revoked only</label>
                            <div role="checkbox" aria-checked={filters.showRevoked} className={'border-[1px] border-solid border-[#48494c] h-[14px] w-[14px] rounded-[3px] flex justify-center items-center' + (!filters.showRevoked ? ' bg-[#eef0f2]' : ' bg-[#48494c]') + (allowances && isConnected ? ' cursor-pointer' : '')}>
                                {filters.showRevoked && <img alt="checkbox image" src={checked}/>}
                            </div>
                        </div>
                        <div onClick={allowances && isConnected ? handleClearFilter :undefined} className={'flex justify-center items-center gap-x-[10px] bg-[hsl(210,25%,100%)] px-[15px] rounded-[6px] shadow-[0_1px_2px_#A8B0BD10,0_3px_6px_#5D81B930]' + (allowances && isConnected ? ' cursor-pointer' : '')}>
                            <label id='clearFiltersLabel' className={'flex items-center text-[14px]' + (allowances && isConnected ? ' cursor-pointer' : '')}>Clear filters</label>
